...
void xsapi_thread_pool::set_thread_ideal_processor(_In_ int threadIndex, _In_ PPROCESSOR_NUMBER lpIdealProcessor)
{
    if (threadIndex == -1)
    {
        for (int i = 0; i < m_numActiveThreads; i++)
        {
            ...
            if (hThread != nullptr)
            {
                SetThreadIdealProcessorEx(hThread, lpIdealProcessor, nullptr);
            }
        }

        m_defaultIdealProcessor.Group = lpIdealProcessor->Group;
        m_defaultIdealProcessor.Number = lpIdealProcessor->Number;
    }
    else
    {
